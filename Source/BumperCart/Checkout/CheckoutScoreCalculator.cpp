...
int32 UCheckoutScoreCalculator::CalculateTotalScore(int32 BaseScore, int32 SaleBonusScore, int32 LastCheckoutBonusScore)
{
    const int32 TotalScore = BaseScore + SaleBonusScore + LastCheckoutBonusScore;
...
FCheckoutScoreResult UCheckoutScoreCalculator::CalculateCheckoutScore(const TArray<FLoadedProductInfo>& Products, float SaleBonusMultiplier, bool bIsLastCheckoutBonusApplied, float LastCheckoutBounusMultiplier)
{
    ...
    Result.TotalScore = CalculateTotalScore(Result.BaseScore, Result.SaleBonusScore, Result.LastCheckoutBonusScore);
